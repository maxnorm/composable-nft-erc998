...
describe("ERC998 contract", function () {
  ...
  describe("NFT Composition", function () {
    ...
    describe("Error Cases", function () {
      describe("ERC721 Operations", function () {
        ...
        it("should revert when approving non-existent token", async function () {
          const nonExistentTokenId = 999;
          
          await expect(erc998.approve(bob.address, nonExistentTokenId))
            .to.be.revertedWithCustomError(erc998, "ERC721NonexistentToken");
        });

        it("should revert when getting approved for non-existent token", async function () {
          const nonExistentTokenId = 999;
          
          await expect(erc998.getApproved(nonExistentTokenId))
            .to.be.revertedWithCustomError(erc998, "ERC721NonexistentToken");
        });
      });
  ...
  describe("ERC20 Support", function () {
    ...
    describe("ERC20 Transfer", function () {
      ...
      it("should revert when transferring more ERC20 than available", async function () {
        const { ERC998TokenId } = await mintFixture();
        const transferAmount = ethers.parseUnits("100", 18);
        ...
        const excessiveAmount = ethers.parseUnits("150", 18);
        
        await expect(
          erc998.transferERC20(ERC998TokenId, alice.address, erc20_1_address, excessiveAmount)
        ).to.be.revertedWithCustomError(erc998, "ERC998_InsufficientERC20Balance");
      });
    });
